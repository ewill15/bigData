...
Router.get('/demo', function(req, res) {
  Usuarios.find({user: req.query.user}).count({}, function(err, count) { 
    if(count>0){ //Si el registro es mayor a 0
        res.send("usuario: demo | password:123456") 
    }else{
      Eventos.find({}).count({}, function(err, count) { //Que no exista ningún evento creado en la base de datos
      ...
      Operaciones.crearUsuarioDemo((error, result) => { //Si no , llamar la función crearUsuarioDemo
        if(error){
